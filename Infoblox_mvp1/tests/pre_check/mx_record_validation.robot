*** Settings ***
Documentation     MX Record validation tests - supports both add and delete operations
...
${JSON_FILE}              ${CURDIR}/../../prod_changes/${GRID_HOST}/mx_record.json
${COUNTER_FILE}           ${CURDIR}/../../robot_reports/pre_check/execution_counter.json
${OPERATION_TYPE}         add

*** Test Cases ***
Validate MX Record JSON File Exists
    [Documentation]    Verify that the MX record JSON file exists
    [Tags]    mx_record    validation    file_check
    File Should Exist    ${JSON_FILE}
    Log    ✓ MX record JSON file found    INFO

Validate MX Record Required Fields
    [Documentation]    Verify all MX records have required fields
    [Tags]    mx_record    validation    required_fields
    ${records}=    Load JSON Records    ${JSON_FILE}

    FOR    ${record}    IN    @{records}
        Dictionary Should Contain Key    ${record}    name    msg=Missing 'name' field
        Dictionary Should Contain Key    ${record}    mail_exchanger    msg=Missing 'mail_exchanger' field
        Dictionary Should Contain Key    ${record}    preference    msg=Missing 'preference' field
        Dictionary Should Contain Key    ${record}    view    msg=Missing 'view' field
        Log    ✓ Record '${record['name']}' has all required fields    INFO
    END

Verify MX Record Existence Based On Operation
    [Documentation]    For ADD: Fail if records exist. For DELETE: Fail if records don't exist
    [Tags]    mx_record    validation    existence_check
    ...
    FOR    ${record}    IN    @{records}
        ...
        ${existing}=    Get MX Records    name=${name}    view=${view}
