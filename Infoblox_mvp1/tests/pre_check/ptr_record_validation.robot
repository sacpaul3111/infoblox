*** Settings ***
Documentation     PTR Record validation tests - supports both add and delete operations
...
${JSON_FILE}              ${CURDIR}/../../prod_changes/${GRID_HOST}/ptr_record.json
${COUNTER_FILE}           ${CURDIR}/../../robot_reports/pre_check/execution_counter.json
${OPERATION_TYPE}         add

*** Test Cases ***
Validate PTR Record JSON File Exists
    [Documentation]    Verify that the PTR record JSON file exists
    [Tags]    ptr_record    validation    file_check
    File Should Exist    ${JSON_FILE}
    Log    ✓ PTR record JSON file found    INFO

Validate PTR Record Required Fields
    [Documentation]    Verify all PTR records have required fields
    [Tags]    ptr_record    validation    required_fields
    ${records}=    Load JSON Records    ${JSON_FILE}

    FOR    ${record}    IN    @{records}
        Dictionary Should Contain Key    ${record}    name    msg=Missing 'name' field
        Dictionary Should Contain Key    ${record}    ptrdname    msg=Missing 'ptrdname' field
        Dictionary Should Contain Key    ${record}    view    msg=Missing 'view' field
        Log    ✓ Record '${record['name']}' has all required fields    INFO
    END

Verify PTR Record Existence Based On Operation
    [Documentation]    For ADD: Fail if records exist. For DELETE: Fail if records don't exist
    [Tags]    ptr_record    validation    existence_check
    ...
    FOR    ${record}    IN    @{records}
        ...
        ${existing}=    Get PTR Records    name=${name}    view=${view}
