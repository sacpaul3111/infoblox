*** Settings ***
Documentation     Alias Record validation tests - supports both add and delete operations
...
${JSON_FILE}              ${CURDIR}/../../prod_changes/${GRID_HOST}/alias_record.json
${COUNTER_FILE}           ${CURDIR}/../../robot_reports/pre_check/execution_counter.json
${OPERATION_TYPE}         add

*** Test Cases ***
Validate Alias Record JSON File Exists
    [Documentation]    Verify that the Alias record JSON file exists
    [Tags]    alias_record    validation    file_check
    File Should Exist    ${JSON_FILE}
    Log    ✓ Alias record JSON file found    INFO

Validate Alias Record Required Fields
    [Documentation]    Verify all Alias records have required fields
    [Tags]    alias_record    validation    required_fields
...
Verify Alias Record Existence Based On Operation
    [Documentation]    For ADD: Fail if records exist. For DELETE: Fail if records don't exist
    [Tags]    alias_record    validation    existence_check
    ...
    FOR    ${record}    IN    @{records}
        ...
        ${existing}=    Get Alias Records    name=${name}    view=${view}
