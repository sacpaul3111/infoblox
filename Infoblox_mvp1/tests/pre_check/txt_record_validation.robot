*** Settings ***
Documentation     TXT Record validation tests - supports both add and delete operations
...
${JSON_FILE}              ${CURDIR}/../../prod_changes/${GRID_HOST}/txt_record.json
${COUNTER_FILE}           ${CURDIR}/../../robot_reports/pre_check/execution_counter.json
${OPERATION_TYPE}         add

*** Test Cases ***
Validate TXT Record JSON File Exists
    [Documentation]    Verify that the TXT record JSON file exists
    [Tags]    txt_record    validation    file_check
    File Should Exist    ${JSON_FILE}
    Log    ✓ TXT record JSON file found    INFO

Validate TXT Record Required Fields
    [Documentation]    Verify all TXT records have required fields
    [Tags]    txt_record    validation    required_fields
    ${records}=    Load JSON Records    ${JSON_FILE}

    FOR    ${record}    IN    @{records}
        Dictionary Should Contain Key    ${record}    name    msg=Missing 'name' field
        Dictionary Should Contain Key    ${record}    text    msg=Missing 'text' field
        Dictionary Should Contain Key    ${record}    view    msg=Missing 'view' field
        Log    ✓ Record '${record['name']}' has all required fields    INFO
    END

Verify TXT Record Existence Based On Operation
    [Documentation]    For ADD: Fail if records exist. For DELETE: Fail if records don't exist
    [Tags]    txt_record    validation    existence_check
    ...
    FOR    ${record}    IN    @{records}
        ...
        ${existing}=    Get TXT Records    name=${name}    view=${view}
