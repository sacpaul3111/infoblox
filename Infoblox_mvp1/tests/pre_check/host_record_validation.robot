*** Settings ***
Documentation     Host Record validation tests - supports both add and delete operations
...
${JSON_FILE}              ${CURDIR}/../../prod_changes/${GRID_HOST}/host_record.json
${COUNTER_FILE}           ${CURDIR}/../../robot_reports/pre_check/execution_counter.json
${OPERATION_TYPE}         add

*** Test Cases ***
Validate Host Record JSON File Exists
    [Documentation]    Verify that the Host record JSON file exists
    [Tags]    host_record    validation    file_check
    File Should Exist    ${JSON_FILE}
    Log    ✓ Host record JSON file found    INFO

Validate Host Record Required Fields
    [Documentation]    Verify all Host records have required fields
    [Tags]    host_record    validation    required_fields
...
Verify Host Record Existence Based On Operation
    [Documentation]    For ADD: Fail if records exist. For DELETE: Fail if records don't exist
    [Tags]    host_record    validation    existence_check
    ...
    FOR    ${record}    IN    @{records}
        ...
        ${existing}=    Get Host Records    name=${name}    view=${view}
