*** Settings ***
Documentation     AAAA Record post-deployment verification - supports both add and delete operations
...
Verify AAAA Records Operation Result
    ...
    IF    '${OPERATION_TYPE}' == 'add'
        ...
        FOR    ${record}    IN    @{records}
            ...
            ${expected_ip}=    Set Variable    ${record['ipv6addr']}
            ...
            ELSE
                ${actual_ip}=    Set Variable    ${existing[0]['ipv6addr']}
                IF    '${actual_ip}' == '${expected_ip}'
                    Log    ✓ Record '${name}' verified with IPv6 ${actual_ip}    INFO
                    ${verified}=    Evaluate    ${verified} + 1
                ELSE
                    Log    ✗ Record '${name}' has wrong IPv6: expected ${expected_ip}, got ${actual_ip}    ERROR
                    Append To List    ${failed}    ${name}
                END
            END
        END
    ...
    Should Be Equal As Numbers    ${failed_count}    0    msg=${OPERATION_TYPE} operation failed for ${failed_count} record(s): ${failed}
